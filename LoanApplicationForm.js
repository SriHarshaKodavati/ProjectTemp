...
const LoanApplicationForm = () => {
  ...
  // Create new application when component mounts
  useEffect(() => {
  ...
  const createNewApplication = async () => {
    try {
      ...
      console.log('New application created with ID:', application.id);
    } catch (error) {
  ...
  const savePersonalDetails = async (data) => {
    try {
      const response = await fetch(`/api/applications/${applicationId}/personal-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const saveEmploymentDetails = async (data) => {
    try {
      const response = await fetch(`/api/applications/${applicationId}/employment-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          jobTitle: "Not specified", // You might want to add this field to the form
          monthlyIncome: parseFloat(data.monthlyCompanyIncome) || 0,
          unemploymentIncome: parseFloat(data.unemploymentIncome) || 0,
          yearsOfExperience: parseInt(data.totalWorkExperience) || 0,
  ...
  const saveLoanDetails = async (data) => {
    try {
      const response = await fetch(`/api/applications/${applicationId}/loan-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const saveDocumentDetails = async (files) => {
    try {
      // For now, just save file names. In a real application, you'd upload files first
      const response = await fetch(`/api/applications/${applicationId}/document-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          salarySlipPath: files.salarySlips ? files.salarySlips.name : null,
          bankStatementPath: files.bankStatements ? files.bankStatements.name : null,
  ...
  const saveExistingLoanDetails = async (data) => {
    try {
      // Only save if there's actual data
      if (!data.existingLoanFullName && !data.existingLoanType && !data.existingLoanLender) {
        return; // Skip if no existing loan data
      }

      const response = await fetch(`/api/applications/${applicationId}/existing-loan-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error('Failed to save existing loan details');
      }
      
      return await response.json();
    } catch (error) {
      console.error('Error saving existing loan details:', error);
      throw error;
    }
  };

  const saveReferenceDetails = async (data) => {
    try {
      const response = await fetch(`/api/applications/${applicationId}/reference-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleNextStep = async () => {
    if (!applicationId) {
      alert('Please wait for the application to be created');
      return;
    }
      ...
      try {
        ...
        switch (currentStepData.key) {
          ...
          case "employment":
            ...
            break;
          case "loan":
            await saveLoanDetails(formData);
            break;
          case "documents":
            await saveDocumentDetails(uploadedFiles);
            break;
          case "existing":
            await saveExistingLoanDetails(formData);
            break;
          case "references":
            await saveReferenceDetails(formData);
            break;
        }
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "totalWorkExperience":
        ...
        else if (isNaN(parseFloat(value)) || parseFloat(value) <= 0) error = "Total work experience should be greater than 0 years";
        break;
      ...
      case "unemploymentIncome":
        if (value && (isNaN(value) || parseFloat(value) < 0)) error = "Please enter a valid positive amount";
        break;
      case "officeAddress":
